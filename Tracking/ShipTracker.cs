...
{
    internal sealed class ShipTracker
    {
        ...
        public ShipTracker(PluginConfig config, ITelemetrySink sink, RecentHitTracker hits)
        {
        ...
        {
            if (!_config.Ships || !Qualifies(zdo, info) || !_built.Add(zdo.m_uid)) return;
            Emit("ship_built", zdo, info, PlayerUtil.FromCreator(zdo.GetLong(ZDOVars.s_creator, 0L)));
        }

        public void Destroyed(ZDO zdo, PrefabInfo info)
        {
            if (!_config.Ships || !Qualifies(zdo, info) || !_destroyed.Add(zdo.m_uid)) return;
            Emit("ship_destroyed", zdo, info, _hits.Resolve(zdo.m_uid, 0.75f));
        }

        private static bool Qualifies(ZDO zdo, PrefabInfo info) => info != null && info.IsShip && zdo.GetLong(ZDOVars.s_creator, 0L) != 0L;

        private void Emit(string name, ZDO zdo, PrefabInfo info, PlayerIdentity player)
        {
            var telemetryEvent = new TelemetryEvent()
                .Add("ship_type", info.Type)
                .Add("ship_display_name", info.DisplayName);
