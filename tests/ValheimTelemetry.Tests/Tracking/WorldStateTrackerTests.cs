...
public sealed class WorldStateTrackerTests
{
    ...
    [Fact]
    public void KeySet_ExistingValuelessKey_DoesNotEmit()
    {
        // Arrange
        var sink = new Mock<ITelemetrySink>(MockBehavior.Strict);
        var capturedEvents = new List<TelemetryEvent>();
        ...
        // Act
        sut.KeySet("defeated_eikthyr", true, null!);
    ...
    [Fact]
    public void KeyRemoved_EmptyPreviousValue_EmitsNullPreviousValue()
    {
        // Arrange
        var sink = new Mock<ITelemetrySink>(MockBehavior.Strict);
        TelemetryEvent? capturedEvent = null;
        ...
        // Act
        sut.KeyRemoved("defeated_eikthyr", string.Empty);

        // Assert
        capturedEvent.Should().NotBeNull();
        capturedEvent!.Properties.Should().Contain(
            new KeyValuePair<string, object>("previous_value", null!));
        sink.Verify(candidate => candidate.Emit(It.IsAny<string>(), It.IsAny<TelemetryEvent>()), Times.Once);
    }

    [Fact]
    public void KeyRemoved_DisabledTracker_DoesNotEmit()
    {
        // Arrange
        var sink = new Mock<ITelemetrySink>(MockBehavior.Strict);
        var capturedEvents = new List<TelemetryEvent>();
        ...
        // Act
        sut.KeyRemoved("defeated_eikthyr", null!);
    ...
    [Fact]
    public void KeyRemoved_EmptyKey_DoesNotEmit()
    {
        // Arrange
        var sink = new Mock<ITelemetrySink>(MockBehavior.Strict);
        var capturedEvents = new List<TelemetryEvent>();
        ...
        // Act
        sut.KeyRemoved(string.Empty, null!);
